...
parser.add_argument('-d', '--days', type=str, default='1',
                    help='1 7 30 90 180 365 all')
#
...
parser.add_argument("--num_contexts", type=int, default=200,
                    help="# contexts for interaction")
parser.add_argument('--max_contexts', type=int,
                    default=1000, help='max contexts to look at')
...
parser.add_argument("--word_embed_dim", type=int, default=100,
                    help="embedding dimention for word")
parser.add_argument('--node_embed_dim', type=int, default=128)
parser.add_argument("--dropout", type=float, default=0,
                    help="size of testing set")
parser.add_argument('--bi_out_dim', type=int, default=50,
                    help='dim for the last bilinear layer for output')
# model selection
...
args.restore_para_file = './saved_models/{0}_{1}_pretrain_model_dict.pkl'.format(
    args.per, args.days)
# path to store
args.save_dir = './saved_models/rank_model_per{0}_{1}'.format(
    args.per, args.days)
# print(args.save_dir)
...
args.term_concept_dict = pickle.load(
    open('../data/mappings/term_concept_mapping.pkl', 'rb'))
args.concept_term_dict = pickle.load(
    open('../data/mappings/concept_term_mapping.pkl', 'rb'))
args.all_iv_terms = pickle.load(open(
    '../data/sym_data/all_iv_terms_per{0}_{1}.pkl'.format(args.per, args.days), 'rb'))
args.neighbors = []
...
train_graph = nx.read_gpickle(
    '../data/sym_data/train_graph_nx_per' + args.per + '_' + args.days + '.gpickle')
test_graph = nx.read_gpickle(
    '../data/sym_data/test_graph_nx_per' + args.per + '_' + args.days + '.gpickle')
...
all_oov_test = pickle.load(open(
    '../data/sym_data/oov_test_dict_per' + args.per + '_' + args.days + '.pkl', 'rb'))
...
reuse_oov_path = '../data/sym_data/dev_test_data_per{0}_{1}_{2}.pkl'.format(
    args.per, args.days, args.num_oov)
if os.path.exists(reuse_oov_path):
    dev, iv_test, dis_iv_test, oov_test, dis_oov_test = pickle.load(
        open(reuse_oov_path, 'rb'))
else:
    print('Re-sample negative samples for testing!')
    dev = utils.random_neg_sampling(
        dev_pos, args.all_iv_terms, args.term_concept_dict, args.test_neg_num)

    iv_test = utils.random_neg_sampling(
        iv_test_pos, args.all_iv_terms, args.term_concept_dict, args.test_neg_num)
    dis_iv_test = utils.random_neg_sampling(
        dis_iv_pos, args.all_iv_terms, args.term_concept_dict, args.test_neg_num)

    oov_test = utils.random_neg_sampling(
        oov_test_pos, args.all_iv_terms, args.term_concept_dict, args.test_neg_num)
    dis_oov_test = utils.random_neg_sampling(
        dis_oov_pos, args.all_iv_terms, args.term_concept_dict, args.test_neg_num)

    pickle.dump([dev, iv_test, dis_iv_test, oov_test, dis_oov_test],
                open(reuse_oov_path, 'wb'), protocol=-1)
...
# data pre-processing
restore_para_file = './saved_models/{0}_{1}_pretrain_model_dict.pkl'.format(
    args.per, args.days)
...
print(len(dev_idx), len(iv_test_idx), len(dis_iv_test),
      len(oov_test_idx), len(dis_oov_test_idx))
...
for epoch in range(args.num_epochs):
    ...
    for cur_sample in train_pos:
        ...
        cur_raw_data = utils.random_neg_sampling(
            [cur_sample], args.all_iv_terms, args.term_concept_dict, args.train_neg_num)
        cur_idx_data = utils.make_idx_data(cur_raw_data, args)
        t1_list, t2_list, onehot_labels = utils.preprocess(
            cur_idx_data[0], args)
    ...
    utils.adjust_learning_rate(
        optimizer, args.learning_rate / (1 + (epoch + 1) * args.lr_decay))

    if epoch % args.test_interval == 0:
        all_dev = train_utils.evaluation(dev_idx, model, criterion, args)
        print("Epoch-{0}: All Dev {1}: {2:.5}".format(epoch,
              args.metric.upper(), all_dev))
        ...
        if all_dev > best_on_dev:
            ...
            all_iv = train_utils.evaluation(
                iv_test_idx, model, criterion, args)
            print(
                "--- Testing: All IV Test {0}: {1:.5}".format(args.metric.upper(), all_iv))

            all_iv = train_utils.evaluation(
                dis_iv_test_idx, model, criterion, args)
            print(
                "--- Testing: All Dis IV Test {0}: {1:.5}".format(args.metric.upper(), all_iv))

            all_oov = train_utils.evaluation(
                oov_test_idx, model, criterion, args)
            print(
                "--- Testing: All OOV Test {0}: {1:.5}".format(args.metric.upper(), all_oov))

            all_oov = train_utils.evaluation(
                dis_oov_test_idx, model, criterion, args)
            print(
                "--- Testing: All Dis OOV Test {0}: {1:.5}".format(args.metric.upper(), all_oov))
