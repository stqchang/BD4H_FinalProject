    ...
    def load_glove_vec(embed_filename, vocab, input_dim):
        ...
        with open(embed_filename) as f:
            for idx, line in enumerate(f):
                ...
                word = L[0].lower()
                if word in vocab:
                    word_dict[word] = np.array(
                        L[-input_dim::], dtype=np.float32)
                    # assert(len(word_dict[word]) == input_dim)
        return word_dict

    word_dict = load_glove_vec(
        args.embed_filename, word_to_id.keys(), args.word_embed_dim)

    initrange = 0.5 / args.word_embed_dim
    W = np.random.uniform(-initrange, initrange,
                          (len(word_to_id) + 1, args.word_embed_dim))
    i = 0
...
def w2v_mapping_pretrain(list_phrases, args):
    ...
    word_vocab = [x[0] for x in full_word_vocab.items(
    ) if x[0] in pretrain_word_vocab or x[1] > 20]
    ...
    print('Find {0} words with pretrain ratio: {1}'.format(
        len(word_to_id), i / float(len(word_to_id))))
...
def g2v_mapping_pretrain(list_words, args):
    ...
    ngrams_vocab = [x[0] for x in full_ngrams_vocab.items(
    ) if x[0] in pretrain_gram_vocab or x[1] > 100]
    ...
    W = np.random.uniform(-initrange, initrange,
                          (len(ngrams_to_id) + 1, args.ngram_embed_dim))
    ...
    print('Find {0} grams with pretrain ratio: {1}'.format(
        len(ngrams_to_id), i / float(len(ngrams_to_id))))
...
def node_mapping(args, node_to_id=None):
    ...
    if not node_to_id:
        terms_mat = np.zeros(
            (len(node_dict) + 1, int(f[0].strip().split()[1])))
        terms_to_idx = {-1: 0}
        idx_to_terms = {0: -1}
